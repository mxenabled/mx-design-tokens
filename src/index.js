import { getTargetSpecificTokens } from 'src/mods'
import { collapseTokens, deepMerge, expandTokens, splitTokens } from 'src/utils'
...
const getTokenSections = (theme = themes.LIGHT, core) => ({
  BackgroundColor: backgroundColor[theme](core),
  BorderColor: borderColor[theme](core),
  BorderRadius: borderRadius[theme](core),
  BoxShadow: boxShadow[theme](core),
  LetterSpacing: letterSpacing[theme](core),
  FontSize: fontSize[theme](core),
  Spacing: spacing[theme](core),
  TextColor: textColor[theme](core),
  ZIndex: zIndex[theme](core),
})
...
export const buildTheme = (theme = themes.LIGHT, target = targets.REACT, tokenOverrides = {}) => {
  ...
  const tokenList = expandTokens(tokenOverrides)
  const partition = splitTokens(tokenList)
  const coreTokens = collapseTokens(partition.coreTokens)
  // const leafTokens = collapseTokens(partition.leafTokens)
  const targetSpecificCore = getTargetSpecificTokens(target, coreTokens)
  const tokenSections = getTokenSections(theme, targetSpecificCore)
  const builtTokens = deepMerge(targetSpecificCore, tokenSections)
  // const finalTokens = deepMerge(builtTokens, leafTokens)
